...
def residual_module(layer_in, n_filters):
    # conv1,
    init = RandomNormal(stddev=0.02)
    conv1 = Conv2D(n_filters, (4,4), padding='same', activation='relu', kernel_initializer=init)(layer_in)
    conv1 = BatchNormalization()(conv1, training=True)
    conv1 = LeakyReLU(alpha=0.2)(conv1)
    # conv2
    conv2 = Conv2D(n_filters, (4,4), padding='same', activation='linear', kernel_initializer=init)(conv1)
...
def resnet_generator(image_shape=(256,256,1)):
    # weight initialization
    init = RandomNormal(stddev=0.02)
    # image input
    ...
    d2 = resnet_decoder(d1,128)
    d3 = resnet_decoder(d2, 64)
    g = Conv2DTranspose(1, (4, 4), strides=(1, 1), padding='same', kernel_initializer=init)(d3)
...
g_model = resnet_generator((256,256,1))
